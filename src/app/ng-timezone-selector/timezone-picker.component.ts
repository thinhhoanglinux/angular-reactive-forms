...
export const TIMEZONE_PICKER_VALUE_ACCESSOR: any = {
...
@Component({
  ...
  providers: [TIMEZONE_PICKER_VALUE_ACCESSOR]
})
export class TimezonePickerComponent implements AfterViewInit, ControlValueAccessor {
  ...
  private triggerChangeEvent(): void {
    ...
    this.writeValue(this.currentTimezone);
  }
  ...
  offsetOfTimezone(zone: string): string {
    ...
    const offSetTimezone = `(GMT${neg ? '-' : '+'}${this.rjust(
      hours.toString(),
      2
